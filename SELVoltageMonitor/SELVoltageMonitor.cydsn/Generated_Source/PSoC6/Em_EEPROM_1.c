...
*/
cy_stc_eeprom_context_t Em_EEPROM_1_context;
...
/* For PSoC 6 place EEPROM into Emulated EEPROM flash area */
    ...
    const uint8_t Em_EEPROM_1_em_EepromStorage[Em_EEPROM_1_ACTUAL_SIZE] = {0u};

#endif /* ((CY_PSOC6) && (Em_EEPROM_1_STORAGE_EM_EEPROM)) */


/*******************************************************************************
* Function Name: Em_EEPROM_1_Init
********************************************************************************
*
...
cy_en_em_eeprom_status_t Em_EEPROM_1_Init(uint32 startAddress)
{
    ...
    config.blockingWrite =      Em_EEPROM_1_config.blockingWrite;
    config.eepromSize =         Em_EEPROM_1_config.eepromSize;
    config.redundantCopy =      Em_EEPROM_1_config.redundantCopy;
    config.wearLevelingFactor = Em_EEPROM_1_config.wearLevelingFactor;
    
#if ((CY_PSOC6) && (Em_EEPROM_1_STORAGE_EM_EEPROM))
    /* For PSoC 6, if Emulated EEPROM is selected - override the eeprom start 
    * address.
    */
    config.userFlashStartAddr = (uint32_t)Em_EEPROM_1_em_EepromStorage;
#else
    config.userFlashStartAddr = startAddress;
#endif /* ((CY_PSOC6) && (Em_EEPROM_1_STORAGE_EM_EEPROM)) */
        
    ret = Cy_Em_EEPROM_Init(&config, &Em_EEPROM_1_context);
